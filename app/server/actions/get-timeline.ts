...
// PRODUCTION CACHING: Short cache + on-demand invalidation
export async function getTimeline(
  ...
  try {
    ...
    try {
      const rawTimeline = await getTimelineByTripId(tripId, userId);
      // DEBUG LOGGING - See what the database returns
      ...
      if (!rawTimeline) {
        console.log(`No timeline found for trip ${tripId}`);
        return null;
      }
          ...
          try {
            return {
              ...
              alternatives: (item.alternatives || []).map((alt) => ({
                ...
                startTime: safeDate(alt.startTime),
              ...
              children: (item.children || []).map((child) => ({
                ...
                startTime: safeDate(child.startTime),
                ...
                alternatives: [], // Simplify for children
...
// PRODUCTION UTILITY: Force refresh timeline (useful for debugging)
export async function forceRefreshTimeline(tripId: string) {
  "use server";
  
  revalidateTag(`timeline-${tripId}`);
  console.log(`🔄 Force refreshed timeline cache for tag timeline-${tripId}`);
}